...
def register_fonts(fonts: list[FontSpec], *, logger: logging.Logger | None = None) -> None:
    ...
    for spec in fonts:
        if spec.name in seen_names:
            log.warning(f"Font name '{spec.name}' is duplicated in input (skipped).")
            continue
        ...
        if already_registered:
            log.debug(f"Font '{spec.name}' is already registered (skipped).")
            continue
        try:
            pdfmetrics.registerFont(TTFont(spec.name, spec.file_path.as_posix()))
        except Exception:
            log.error(f"Failed to register font '{spec.name}' from '{spec.file_path}'")
            continue
